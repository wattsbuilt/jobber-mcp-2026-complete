...
if (!apiToken) {
  console.error('FATAL: JOBBER_API_TOKEN environment variable is required');
  process.exit(1);
}
...
function createMcpServer() {
  const client = new JobberClient({ apiToken: apiToken! });
  const server = new Server(
    { name: 'jobber-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
const httpServer = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (req.url === '/health') {
    ...
    res.end(JSON.stringify({ status: 'ok', service: 'jobber-mcp' }));
    return;
  }
...
httpServer.listen(PORT, () => {
  console.log(`Jobber MCP HTTP server listening on port ${PORT}`);
